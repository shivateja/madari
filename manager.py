...
try:
    ...
    if not sys.path.append(pyyaml_dir):
        ...
        print ("Install it by running -" +
            "'python setup.py install' in %s" % pyyaml_dir)
try:
    from crontab import CronTab
except:
    print """python-crontab library import failed.
        You will not be able to add job to crontab. 
        Would you like to continue for generating scripts ? (y/n)"""
    ans = str(raw_input())
    if ans == "y":
        pass
    else:
...
cron_command = "bash %s/hourly.sh" % file_directory
